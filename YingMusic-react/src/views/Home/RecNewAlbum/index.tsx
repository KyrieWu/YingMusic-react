...
const RecNewAlbum: React.FC = () => {
  ...
  let getAltums = async (area: string) => {
    let result = await getNewAlbum(area, 10, 0);
    let playListData = result.albums;
  ...
  return (
    <div className={styles.recAlbumlist_container}>
      ...
      <div className={styles.nav}>
        <a
          onClick={selectAreatoAll}
          className={area == "ALL" ? styles.active : ""}
        >
          精彩推荐
        </a>
        {albumAreas.map((item: AlbumArea) => {
          return (
            <a
              key={item.key}
              className={area == item.key ? styles.active : ""}
              onClick={changeArea}
            >
              {item.value}
            </a>
